...
int main(int argc, char *argv[])
{
    ...
    while (ros::ok())
    {
        ...
        cloud_sub_ptr->ParseData(cloud_data_buff); //将受到的点云数据push到cloud_data_buff的队尾
        ...
        {
            while (cloud_data_buff.size() > 0 && imu_data_buff.size() > 0 && gnss_data_buff.size() > 0)//把cloud imu gnss里面的所有数据都处理干净
            {
                ...
                if (d_time < -0.05)//点云数据时间小，即imu数据是后面来的，这里GPS，IMU，cloud的频率都是10Hz
                {
                    cloud_data_buff.pop_front();
                }
                else if (d_time > 0.05)//点云数据时间大
                {
                    imu_data_buff.pop_front();//因为imu数据是和gnss数据一起得到的
                    gnss_data_buff.pop_front();
                }
                else//点云数据时间与imu时间差距在50ms
                {
                    cloud_data_buff.pop_front();
                    ...
                    gnss_data.UpdateXYZ();
                    odometry_matrix(0, 3) = gnss_data.local_E;// 平移
                    odometry_matrix(1, 3) = gnss_data.local_N;
                    odometry_matrix(2, 3) = gnss_data.local_U;
                    odometry_matrix.block<3, 3>(0, 0) = imu_data.GetOrientationMatrix();//利用imu得到的四元数旋转信息得到旋转矩阵
                    odometry_matrix *= lidar_to_imu;

                    pcl::transformPointCloud(*cloud_data.cloud_ptr, *cloud_data.cloud_ptr, odometry_matrix);//点云旋转
